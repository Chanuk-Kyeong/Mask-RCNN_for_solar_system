...
def display_instances(image, boxes, masks, class_ids, class_names,
                      ...
                      colors=None, captions=None, path=None,RATIO_M=None):
    """
    ...
    ratio_m = RATIO_M # 25cm 급
    # ratio_m = 0.12 # 12cm 급
    RATIO_PIXEL_TO_SQUARE_M = ratio_m**2
    ...
    for i in range(N):
        ...
        if not captions:
            ...
            area = cv2.contourArea(contours_mask[0])
            area_m = round(area * RATIO_PIXEL_TO_SQUARE_M, 1)
            caption = "{}:{}m²".format(label[:5], area_m) if score else label
        else:
            caption = captions[i]
...
def plot_precision_recall(AP, precisions, recalls):
    ...
    _, ax = plt.subplots(1)
    ax.set_title("Precision-Recall Curve. AP@70 = {:.3f}".format(AP))
    ax.set_ylim(0, 1.1)
    ax.set_xlim(0, 1.1)
    _ = ax.plot(recalls, precisions)
    
def save_precision_recall(AP, precisions, recalls,path):
    """Draw the precision-recall curve.
